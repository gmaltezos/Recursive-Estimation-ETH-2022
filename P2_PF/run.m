...
if doplot
    ...
    contour(1, 2) = contour(1, 2) + state(1, 4);
    contour(2, 2) = contour(2, 2) + state(1, 4);
    contour(8, 1) = contour(8, 1) + state(1, 5);
    contour(9, 1) = contour(9, 1) + state(1, 5);
    axis([min(contour(:,1))-spacing,max(contour(:,1))+spacing,...
          min(contour(:,2))-spacing,max(contour(:,2))+spacing])
    hold all
    particle_walls_12 = plot(NaN,NaN,'r');
    particle_walls_12.XData = reshape( ...
        [contour(1,1) * ones(1, N_particles); ...
         contour(2,1) * ones(1, N_particles); ...
         NaN(1, N_particles)], 1, 3 * N_particles);
    particle_walls_89 = plot(NaN,NaN,'r');
    particle_walls_89.YData = reshape( ...
    ...
    for k = 1:size(state,1)
        ...
        particle_walls_12.YData = reshape( ...
            [rho(k,:) + simConst.contour(1,2); ...
             rho(k,:) + simConst.contour(2,2); ...
             NaN(1, N_particles)], 1, 3 * N_particles);
        particle_walls_89.XData = reshape( ...
